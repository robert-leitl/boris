...
/**
 * Generates points inside a box volume using possion disk sampling.
 * 
 * @see https://www.cs.ubc.ca/~rbridson/docs/bridson-siggraph07-poissondisk.pdf
 */
export class PoissonBox {
    ...
    constructor(width, height, depth, radius) {
        this.radius = radius;
        this.boxSize = new Vector3(width, height, depth);
        this.box = new Box3(
            new Vector3(),
            this.boxSize.clone()
        );
    ...
    generateSamples() {
        ...
        // find initial sample
        let p = new Vector3(
            randomInRange(0, this.boxSize.x),
            randomInRange(0, this.boxSize.y),
            randomInRange(0, this.boxSize.z),
        );
